# Integrations: turn tracking, the local proxy, Claude Code and Codex

These integrations observe live traffic and account for it. They use the same
pricing data as the provider-agnostic API described in the
[README](../README.md): OpenAI-protocol traffic is priced from the legacy
three-bucket view of the catalog, and Anthropic Messages traffic from the
Anthropic catalog entries (with separate cache-write and cache-read prices).
...
The proxy-backed integration below is independent: it routes Claude Code's Anthropic Messages traffic through the local OCC proxy, prices every request from this project's Anthropic pricing catalog, and shows both the current-turn and cumulative-session cost that OCC observed.
